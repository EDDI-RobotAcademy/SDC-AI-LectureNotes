...
void vector_example()
{
    Vector v1;
    Vector v2;
// v1.operator+(v2) -> operator+ 라는 함수를 + 로 간단히 대체해서 사용할 수 있게 만듬
    Vector sum = v1 + v2;
// v2.operator-(v2)
    Vector sub = v1 - v2;

// operator<< 이긴하지만
// 만들다 보니 printVector()로 만들어 놓는 게 나을 거 같다.
...
void matrix_example()
{
    ...
    // n by m * m by p = n by p (곱하는 열과 뒤의 행이 같아야 곱셈이 가능함)
    // 2 by 2
