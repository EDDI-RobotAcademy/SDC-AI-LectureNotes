...
bool DbProcess::insertBoardData(const std::string& content, const std::string& title, const std::string& writer) {
    std::string insertQuery = "INSERT INTO board (content, title, writer, reg_date, upd_date) VALUES ('" +
            content + "', '" + title + "', '" + writer + "',NOW(6), NOW(6))";
...
bool DbProcess::updateAccountData(unsigned int accountId, const std::string& newAccount_id, const std::string& newPassword) {
    std::string updateQuery = "UPDATE account SET account_id = '" + newAccount_id +
...
bool DbProcess::updateBoardData(unsigned int boardId, const std::string& newTitle, const std::string& newContent) {
...
bool DbProcess::deleteAccountData(int accountId) {
    std::string deleteQuery = "DELETE FROM account WHERE id = " + std::to_string(accountId);
...
void DbProcess::readAccountData(int accountId) {
    ...
    std::string readQuery = "SELECT * FROM account WHERE id = " + std::to_string(accountId);
    if (mysql_query(conn, readQuery.c_str()) == 0) {
        ...
        if (row != nullptr) {
            ...
            std::cout << "Record not found for id: " << accountId << std::endl;
        }
