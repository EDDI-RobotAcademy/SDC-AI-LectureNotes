...
bool DbProcess::updateDataBoard(int boardId, const std::string& newTitle, const std::string& newContent) {
...
bool DbProcess::updateDataAccount(int uid, const std::string& newName, const std::string& newPassword) {
...
bool DbProcess::deleteAccountData(int uid) {
...
void DbProcess::readAccountData(int uid) {
    // 읽기의 경우도 사실은 select 기반
    // 다만 조건을 where로 줍니다.
    // where board_id가 몇이냐를 기준으로 합니다.
    // 우리가 직접 만들었던 부분에서 이 녀석은 결국 unique id 입니다.
