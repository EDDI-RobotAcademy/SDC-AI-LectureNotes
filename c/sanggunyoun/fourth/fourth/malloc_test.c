...
{
	// malloc => Memory Allocatoin의 약어
	// 일반적으로 프로그래밍을 할 때 반드시 알아야 하는 4개지 영역이 존재합니다.
	// Stack:	지역 변수
	// Heap:	동적 할당(new, malloc)
	// Data:	전역 변수
	// Text:	기계어 영억

	// malloc을 사용 하면 Heap에 Memory를 할당하게 됩니다.
	// 할당된 메모리의 주소값을 추력(리턴값)으로 뱉어낸다.
	// -상근 동작을 이 후에도 다시 사용가능

	// 포인터 변수의 정의:
	// 특정한 데이터 타입의 메모리 주소 값을 저자아할 수 있는 메모리 공간
	// sizeof(int)는 바이트 수를 가져옵니다 => 4
	// 결론적으로 배열의이름은 배열의 시작 주소고
...
void create_char_dynamic_array(char *name)
{
	...
	// 이것은 강제 캐스팅인데 실제 malloc의 리턴 타입은 void* 이거 때문에 그렇습니다.
...
//void print_char_char_dynamic_array(void)
void print_char_dynamic_array(void)
{
	printf("char_char_dynamic_array = %s\n", char_dynamic_array);
}