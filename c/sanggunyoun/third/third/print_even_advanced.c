...
// EVEN_DECISION(TAGET_NUMBER) 에서 TAGET_NUMBER 는 실제 함수처럼 입력이 되었습니다.
// ((TAGET_NUMBER) % 2== 0) -> 입력은 2로 나눠서 나머지가 0인가요?
// 0이면 참(True), 0이 아니면 거짓(False)

// EVEN_DECISION(TAGEM_NUMBER) 가 ((TAGET_NUMBER) % 2 == 0) 으로 치환됩니다.
