...
// 보편적으로 int 와 '*' 을 별개로 생각하는 경향이 존재합니다.
...
{
	// malloc => Memory Allocation 의 약어
	// 일반적으로 프로그래밍을 할 때 알아야 하는 4가지 영역이 존재합니다.
	// Stack : 지역변수
	// Heap : 동적 할당(new, malloc)
	// Data : 전역 변수
	// Text : 기계어 영역

	// malloc 을 하면 Heap에 Memory를 할당하게 됩니다.
	// Heap 은 동적 할당을 하기 때문에 Stack이 끝나도 사용 할 수 있다. 크기가 가변적이다
	...
	// Heap 에 할당된 메모리 주소를 받아왔으므로
	// Heap 의 영역을 배열처럼 사용 할 수 있게 됩니다.
	// 그리고 malloc 앞에 있는 (int *) 은
	// int 형 포인터로 이 주소를 보겠다는 것을 의미합니다.

	// 또한 malloc(sizeof(int) * count) 는 malloc(4 * 5)로 malloc(20)을 의미합니다.
	// 고로 20byte 의 공간을 Heap에 할당하였고
	// int *로 바라보므로 int 5개 짜리 배열과 동일합니다.
	// int arr[5] 와 같다는 뜻
	// 즉 이러한 형태로 코드를 작성하면
