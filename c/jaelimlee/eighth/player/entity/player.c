...
// 가장 기본적인 Virtual Memory Layout은 아래와 같음
...
{
    // Heap의 할당한 메모리가 어디에 들어가는가? tmp_player
    // 여기서 생성된 것은? 
    // 기능 관점: player 크기 만큼 메모리 할당
    // 도메인 관점: player 객체 생성
    player *tmp_player = (player *)malloc(sizeof(player));
    tmp_player->id = current_player_max++;

    // tmp_player를 return하면 무엇이 return 되는가? 할당된 Heap 메모리 주소
    return tmp_player;
}
...
void set_player_name(player *object, char *name)
{
    ...
    // 기능관점(너무 세부사항 관점으로 문제를 바라보고 있음): char 이름을 할당해서 object->name에 저장하기 위해
    // 기능 관점의 추가적 확장: 포인터는 반드시 메모리를 할당하고 사용해야 함.
    // 도메인 관점: object->name에 문자열 객체를 생성하기 위해
