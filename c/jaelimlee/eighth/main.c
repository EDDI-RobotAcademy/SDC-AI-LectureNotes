...
void player_info_allocation(player **player_object_array, char **name)
{
    ...
    for (i = 0; i < MAX_PLAYER; i++)
    {
        ...
        // player 객체의 정보 출려 <- Java 따윈 모를 경우
        // 여기에 무엇을 전달하는가?

        // 세부 사항: 메모리 주소값 
        // 도메인: player 객체 
        // 결론적으로 아무리 복잡한 상황이더라도 직관성을 제공하는 것이 Domain(도메인) 관점.
...
void dice_memory_allocation(dice *dice_array[MAX_PLAYER][MAX_DICE])
{
    ...
    for (i = 0; i < MAX_PLAYER; i++)
    {
        for (j = 0; j < MAX_DICE; j++)
        {
            ...
            // 그래서 init_ 붙은 계열을 전부 C++, Java 등의 생성자와 동일하게 여기라고 함.
...
void play_dice_game(dice *dice_array[MAX_PLAYER][MAX_DICE])
{
    ...
    for (i = 0; i < MAX_PLAYER; i++)
    {
        ...
        for (j = 0; j < MAX_DICE; j++)
        {
            ...
            // isFirstDiceIsEven을 true, false로 만듬.
            if (j == SECOND_DICE)
            {
                // '!' 는 NOT 연산자로
                // 참을 부정하면 거짓
...
int main (void)
{
    ...
    // 배열의이름은 배열의 시작 주소
    // 포인터는 주소를 저장하는 변수라는 것을 상기할 것.
    // 도메인 관점: player 객체에 name 정보 설정
