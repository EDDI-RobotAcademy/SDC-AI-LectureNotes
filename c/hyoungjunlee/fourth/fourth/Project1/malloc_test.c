...
//보편적으로 int와 '*'을 별개로 생각하느 경향이 존재합니다
//그러나 포인터에서는 'int*'이 데이터타입입니다
...
void create_int_dynamic_array(int count)
{
	...
	//일반적으로 프로그래밍을 할 때 반드시 알아야 하는 4가지 영억이 존재합니다
	//Stack:지역 변수
	...
	// 고로 20바이트의 공간을 Heap에 할당하였고
	// int *로 바라보므로 int 5개의 배열과 동일 합니다
	// 즉 이러한 형태로 코드를 작성하면
