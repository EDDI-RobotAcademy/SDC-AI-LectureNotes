...
void player_info_allocation(player **player_object_array, char **name)
{
    ...
    {
        //init_player_object()에서 할당된 힙 메모리 주소는
        // player_object_array[index]에 가면 무엇이 있는가?
        ...
        //player 객체의 toString()메서드 <-Java스타일 표현
...
void dice_memory_allocation(dice *dice_array[MAX_PLAYER][MAX_DICE])
{
    ...
    for (i = 0; i < MAX_PLAYER; i++)
    {
        for (j = 0; j < MAX_DICE; j++)
        {
            ...
            //그래서 init 붙은 계열을 전부 시플플 자바 등의 생성자와 동일하게 여기라고 했음
            // 결론적으로 내부에서 malloc()을 사용할 것임을 알 수 있음
...
void play_dice_game(dice *dice_array[MAX_PLAYER][MAX_DICE])
{
    ...
    for (i = 0; i < MAX_PLAYER; i++)
    {
        ...
        for (j = 0; j < MAX_DICE; j++)
        {
            //첫 번째 굴린 주사위가 짝수나를 판정하기 위한 변수
            bool isFirstDiceIsEven = false;
            // 두 번째 주사위를 굴릴 때
            // decision even과 get_dice_number를 통해
            // 첫 번째 주사위가 짝수인지 여부를 판단하여
            ...
            // check_first_dice)is_even(j,dice_array);

            // 첫 번쨰 주사위가 짝수임을 판정한 이후 두 번째 주사위가 굴러감
            //첫 번쨰 주사위 또한 여기서 굴러감
