...
void play_game(void)
{
	...
	for (current_player_index = 0; current_player_index < MAX_PLAYER_NUMBER; current_player_index++)
	{
		...
		printf("%d 플레이어 첫번째 주사위 눈금: %d\n", current_player_index,first_dice_number);
...
void apply_dice_skill(int current_player_index)
{
	...
	printf("%d 플레이어 현재 두 번째 주사위 눈금 = %d\n", current_player_index, second_dice_number);
...
void check_winner()
{
	...
	for (i = 0; i < MAX_PLAYER_NUMBER; i++)
	{
		...
		{
			death_count++;
			printf("플레이어%d님은 패배하셨씁니다\n", i);
		}
	}

	if (death_count == 2)
	{
		does_everyone_lose = true;
	}
	if (does_everyone_lose)
	{
		return;
	}
