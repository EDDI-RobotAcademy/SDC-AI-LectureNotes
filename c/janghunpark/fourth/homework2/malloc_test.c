...
* 보편적으로 int와 *을 별개로 생각하는 경향 존재
* 그러나 포인터에서는 'int*'이 데이터 타입임
* int 와 int* 이 존재한다면 두 개는 서로 다른 데이터 타입임
*/
...
void create_int_dynamic_array(int count)
{
	...
	* 일반적으로 프로그래밍을 할 대 반드시 알아야 하는 4가지 영역이 존재한다
	* stack: 지역 변수
