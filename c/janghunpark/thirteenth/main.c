...
// Linux OS가 메모리를 관리할 때 Slab 단위 및 Buddy 단위로 관리함
// Slab은 32 * 2^n 형태로 구성됨
// 그러므로 32, 64, 128, 256, ... 형태로 각을 맞추는게 좋음
// Buddy는 물리 메모리의 최소 단위인 4K (4096바이트) = 2^12 을 기준으로 동작
...
int main (void)
{
    ...
    int created_file_descriptor = file_open(
        "/home/eddi/proj/SDC-AI-LectureNotes/c/janghunpark/thirteenth/created_file/코딩빡시네.txt", O_CREAT | O_RDWR, 0644);

    printf("created_file_descriptor = %d\n", created_file_descriptor);
    // O_CREAT | O_RDWR, 0644 ?? = option이랑 permission 이라고는 나와 있는데,, 알 필요 없겠지?
