...
int file_total_length (int file_descriptor)
{
    ...
    // lseek의 리턴 값은 두 번째 인자부터 번째 인자까지 file pointer를 이동한 숫자
}
    ...
    test_form **test_format_array, int object_count, char *read_contents)
{
    ...
    for (i = 0; i < read_contents_length; i++)
    {
        ...
        // 왜 찾는가?
        // format이 아래와 같아서
        // id:1,major:제갈량,age:20,introduction:화공
...
test_form **read_file_to_format(void)
{
    ...
    read_content_from_file(created_file_descriptor, read_contents); //파일로 부터 content를 읽음
