...
void player_info_allocation(player **player_object_array, char **name)
{
    ...
    {
        // init_player_boject()에서 할당된 Heap Memory 주소는
...
void play_dice_game(dice *dice_array[MAX_PLAYER][MAX_DICE])
{
    ...
    for (i = 0; i < MAX_PLAYER; i++)
    {
        ...
        for (j = 0; j < MAX_DICE; j++)
        {
            ...
            // 두번쨰 주사위를 굴릴 때
            // decision_even과 get dice number를 통해
            // 첫 번쨰 주사위가 짝수인지 여부를 판정하여
            ...
            // check_first_dice_is_even(j, dice_array);로 위 코드를 줄이면 직관성을 줄일 수 있다.

            // 첫 번째 주시위가 짝수임을 판정한 이후 두 번째 주사위가 굴러감
...
int main (void)
{
    // int i, j;
    // player 객체를 관리하는 배열
    // plyer playerArray[MAX_PLAYER]; <- JAVA
    // player는 무엇인가 ?
    // 우리가 커스텀한 데이터타입이다
    ...
    // dice 포인터를 관리한다는 것은 결론적을 무엇을 관리하는 것인가?
    // dice 객체를 관리한다는 것
    dice *dice_array[MAX_PLAYER][MAX_DICE];
